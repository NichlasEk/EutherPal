...
BUILD="$ROOT/build"
DIST="$ROOT/dist"
KEYSTORE="$ROOT/.keystore/eutherpal-tv-dev.jks"
ALIAS="${EUTHERPAL_KEY_ALIAS:-eutherpal-tv}"
...
if [ ! -f "$KEYSTORE" ]; then
  keytool -genkeypair \
    ...
    -keysize 2048 \
    -validity 10000 \
    -dname "CN=EutherPal TV Dev,O=ApothicTech,C=SE"
fi
...
"$BUILD_TOOLS/apksigner" sign \
  ...
  --out "$DIST/eutherpal-tv.apk" \
  "$BUILD/aligned.apk"

"$BUILD_TOOLS/apksigner" verify --verbose "$DIST/eutherpal-tv.apk"
echo "$DIST/eutherpal-tv.apk"
